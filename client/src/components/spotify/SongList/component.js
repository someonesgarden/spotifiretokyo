        ...
        return songs.map((song, i) => {
            ...
            return (
                <li className={song.track.id === songId ? 'active user-song-item' : 'user-song-item'} key={i}>
                    ...
                    <div onClick={() => {
                        (song.track.id === songId) && songPlaying && songPaused ? resumeSong() :
                        ...
                        isrc && (
                            <React.Fragment>
                                <div className={['lyrics', mbtracks[isrc] ? 'searched' : ''].join(' ')}>
                                    ...
                                    {mbtracks[isrc] &&
                                    <p className='mb_info'>{mbtracks[isrc].length} tracks in MusicBrainz.</p>}
                                ...
                                <div className="lyricsBtns">
                                    ...
                                    {
                                        !mbtracks[isrc] &&
                                        <Button variant="contained" style={{backgroundColor: '#1bd6ab'}} size="small"
                                                onClick={() => this.lyricsAction('musicbrainz', song)}>MusicBrainz</Button>
                                    }
                                            ...
                                            <React.Fragment>

                                                {
                                                    ...
                                                    <Button variant="contained"
                                                            style={{backgroundColor: '#d6433b', color: 'white'}}
                                                            size="small"
                                                            onClick={() => this.lyricsAction('musixmatch', isrc)}>MusixMatch</Button>
                                                }

                                                {
                                                    !kglyrics[isrc] &&
                                                    <Button className={kgloading ? "loading" : ""} variant="contained" color="primary" size="small" onClick={() => this.lyricsAction('kget', song)}>Kashi-Get</Button>
                                                }
